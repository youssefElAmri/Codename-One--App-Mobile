...
public class AddBlogForm extends Form {
    ...
    public AddBlogForm(Form previous) {
        ...
        TextArea taContent = new TextArea("", 5, 20);
        ...
        btnAdd.addActionListener(e -> {
            ...
            if (tfTitle.getText().length() == 0 || taContent.getText().length() == 0) {
                ...
                try {
                    ...
                    // Add the Blog object using the ServiceBlog class
                    boolean success = ServiceBlog.getInstance().addBlog(blog);
                    if (success) {
                        Dialog.show("Success", "Blog added successfully", "OK", null);
                        // Navigate back to the previous form
                        previous.showBack();
                    } else {
                        Dialog.show("Error", "Failed to add blog", "OK", null);
                    }
                } catch (Exception ex) {
                    Dialog.show("Error", "Failed to add blog: " + ex.getMessage(), "OK", null);
                }
            }
        });
        
        addAll(tfTitle, taContent, btnImage, btnAdd);
